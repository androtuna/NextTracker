...
import type { TrackableItem, ItemStatus } from '@/types';
...
export default function Dashboard() {
    ...
    const movieItems = items?.filter(item => (item.type === 'movie' || item.type === 'series') && (activeTab === 'all' || item.status === activeTab));
    const bookItems = items?.filter(item => item.type === 'book' && (activeTab === 'all' || item.status === activeTab));
    ...
    return (
        <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-12">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-[var(--foreground)] tracking-tight">{t('dashboard')}</h1>
            ...
            <section className="space-y-6">
                <div className="flex items-center gap-3 border-b border-[var(--border)] pb-4">
                    ...
                    <h2 className="text-2xl font-bold text-[var(--foreground)]">{t('watchingList')}</h2>
                    <span className="ml-auto text-sm text-[var(--muted-foreground)] font-medium bg-[var(--accent)] px-3 py-1 rounded-full">
                        {movieItems?.length || 0} İçerik
                    </span>
                </div>

                {movieItems?.length === 0 ? (
                    <div className="py-12 text-center text-[var(--muted-foreground)] border-2 border-dashed border-[var(--border)] rounded-2xl">
            ...
            <section className="space-y-6">
                <div className="flex items-center gap-3 border-b border-[var(--border)] pb-4">
                    ...
                    <h2 className="text-2xl font-bold text-[var(--foreground)]">{t('readingList')}</h2>
                    <span className="ml-auto text-sm text-[var(--muted-foreground)] font-medium bg-[var(--accent)] px-3 py-1 rounded-full">
                        {bookItems?.length || 0} Kitap
                    </span>
                </div>
...
function DashboardCard({ item }: { item: TrackableItem }) {
    const { t } = useTranslation();

    return (
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-2xl overflow-hidden shadow-sm hover:ring-2 ring-indigo-500/50 transition-all cursor-pointer group flex flex-col">
...
// No redundant imports needed at the bottom
